...
export default function InterviewPage() {
  ...
  useEffect(() => {
    ...
    createInterviewSession(session.user.id, role).then(({ data }) => {
    ...
    <main className="mx-auto max-w-5xl px-6 py-10">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {Object.keys(dataset).map((r) => (
          <button key={r} onClick={() => { setRole(r); setI(0); }} className="rounded-lg border border-white/10 px-3 py-1.5 capitalize hover:bg-white/10">{r}</button>
        ))}
      </div>
      <Card>
        ...
        <div className="mt-3 flex items-center gap-3">
          <Button onClick={submit} disabled={loading || !answer || !sessionId}>{loading ? "Analyzing..." : "Submit Answer"}</Button>
          <span className="text-xs text-white/60">Difficulty: {q.difficulty}</span><span className="ml-auto">⏱️ 02:00</span>
        </div>
      </Card>
