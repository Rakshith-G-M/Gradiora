...
export default function Dashboard() {
  ...
  if (checking || !session) {
    return <main className="mx-auto max-w-6xl px-6 py-10"><div className="glass h-48 rounded-3xl shimmer" /></main>;
  }
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        {[82, 76, 91].map((v, i) => (
          <Card key={i}><p className="text-white/60">Metric {i + 1}</p><p className="text-3xl font-bold">{v}%</p></Card>
        ))}
      </div>
      ...
      <Card className="mt-6">
        ...
        <div className="grid gap-2 md:grid-cols-3">
          {roles.map((r) => (
            <button key={r.value} onClick={() => setSelectedRole(r.value)} className={`rounded-lg border px-3 py-2 text-left transition ${selectedRole === r.value ? "border-secondary bg-cyan-400/10" : "border-white/10 hover:bg-white/10"}`}>
              {r.label}
            </button>
          ))}
        </div>
        <Link href={`/interview?role=${selectedRole}`} className="mt-4 inline-block"><Button>Start Interview Chat</Button></Link>
      </Card>

      <Card className="mt-6">
        <p className="mb-3 font-semibold">Previous Interview Reports</p>
        <div className="space-y-2 text-sm text-white/75">
          {reports.length === 0 ? <p>No reports yet. Complete your first interview.</p> : reports.slice(0, 5).map((r) => <p key={r.id}>Score {r.overall_score}% · {new Date(r.generated_at).toLocaleString()}</p>)}
        </div>
      </Card>
    </main>
  );
